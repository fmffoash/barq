...
جديد (نفس المعاينات الحية) → قايمة قوالب الفئة دي بس (فلترة JS في المتصفح، صفر نداء سيرفر
إضافي) → لون أساسي (`<input type="color">`، محتاج تفعيل بـcheckbox) → خط. لو فؤاد معملش أي
اختيار، كل الحقول دي disabled فمش بتتبعت مع الفورم خالص، والذكاء الاصطناعي بيشتغل بالضبط زي
ما كان (يخمّن الفئة/القالب/الطابع من النص الحر). لو اختار قالب بنفسه،
`AiProjectAssistantService::createFromMessage()` بيتجاهل تخمين الفئة/القالب بالكامل ويستخدم
`suggestContent()` مباشرة على القالب المحدد (نداء Ollama واحد أخف، مش نداء مدموج).
