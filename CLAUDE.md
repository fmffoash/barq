...
**⚠️ قاعدة صيانة تانية (اتحددت 2026-09-21):** لو أي ديبلوي غيّر `routes/web.php` أو
`routes/site.php`، `php artisan route:cache` القديم (لو كان متعمل قبل كده) بيفضل شغّال
بنسخته القديمة وبيخفي أي route جديد تماماً (يرجع 404/`Route not defined` من غير أي خطأ
واضح في اللوج) لحد ما تعمل `php artisan route:clear` (أو `optimize:clear`) صريح — مش كفاية
إنك تعمل `config:cache`/`view:cache` بس وتفتكر إن الديبلوي خلص. **أي ديبلوي فيه تعديل على
...
**⚙️ خيارات "حدد بنفسك" في صفحة الإنشاء (Phase 14، 2026-09-21)** — تحت مربع النص، `<details>`
اختياري بالكامل فيه: نوع النشاط (فئة) → زرار "🖼️ تصفح" يفتح `/templates?category=X` في تاب
جديد (كروت الصورة/اللون التقريبي، شوف Phase 13) → قايمة قوالب الفئة دي بس (فلترة JS في
المتصفح، صفر نداء سيرفر إضافي) → لون أساسي (`<input type="color">`، محتاج تفعيل بـcheckbox)
→ خط (كل خيار معروض بخطه الحقيقي — شوف Phase 15). لو فؤاد معملش أي اختيار، كل الحقول دي
disabled فمش بتتبعت مع الفورم خالص، والذكاء الاصطناعي بيشتغل بالضبط زي ما كان (يخمّن الفئة/
القالب/الطابع من النص الحر). لو اختار قالب بنفسه، `AiProjectAssistantService::
createFromMessage()` بيتجاهل تخمين الفئة/القالب بالكامل ويستخدم `suggestContent()` مباشرة
على القالب المحدد (نداء Ollama واحد أخف، مش نداء مدموج).

**🎨 تحسينات المحرر البصري + معاينة خط حقيقية (Phase 15، 2026-09-21)** — درج "تصميم الموقع"
بقى فيه "🔄 غيّر القالب" فوق (قوالب تانية من نفس فئة القالب الحالي بس، المحتوى بينتقل زي ما
هو — نفس منطق `applyChangeTemplate` بس من غير ذكاء اصطناعي، `GeneratedSiteController::
update()` بيتعامل مع `template_id` اختياري). وكل قايمة اختيار خط في اللوحة (الجزئية
المشتركة `font-select.blade.php` + صفحة إنشاء المشروع) بقى كل خيار فيها معروض بخطه الفعلي
(`style="font-family: var(--font-{key})"`) بدل الاسم بس. **ملحوظة مهمة اتكشفت أثناء التصحيح:**
لو فؤاد يشتكي إن "تغيير الخط العام مش بيأثر"، السبب الشائع مش باج — في الغالب في خانة (زي
hero_title) عندها تخصيص خط خاص بيها بس (`style_overrides_json`، من ضغطة على المحرر البصري
لعنصر معيّن) وده بيغلب الخط العام بـ`!important` عمداً؛ الحل يبان في المحرر نفسه (زرار "✕
إزالة التخصيص" على الخانة دي بالذات)، مش في إعداد الخط العام.
...
**✨ محرر بصري مباشر (WYSIWYG click-to-edit، Phase 12، 2026-09-21)** — `/projects/{project}/
site/live-edit` (route محمي بـ auth) بيرندر الموقع الحقيقي بالظبط وبيخلي أي نص/فقرة قابل
للتعديل بالدوس عليه مباشرة في مكانه (زي فؤاد طلب بالظبط: "زي الفوتوشوب")، مع شريط أدوات
عائم للون/خط الخانة، وزرار "🎨 تصميم الموقع" بيفتح درج فيه نفس منتقيات الألوان/الخط/ترتيب
الأقسام. كل حفظ AJAX جزئي (خانة واحدة بس، مفيش إعادة تحميل صفحة). المسار العام
(`site.show`) صفر تأثير عليه — `$editable` بيتحط `true` بس من `site.live-edit` (شوف
`docs/wysiwyg-editor-plan.md` للتفاصيل الكاملة والقيد الأمني). نص/فقرة بس دلوقتي — صور/
قوايم/روابط لسه من فورم "تعبئة المحتوى" القديم (متاح بجنبه، مش بديل عنه).

**🖼️ صور حقيقية في مكتبة القوالب (Phase 13، خلصت 2026-09-21)** — كل الـ20 فئة (300 قالب)
بقى ليها 4 صور حقيقية (هيرو + 3 معرض) كـ default_value لخانات الصور
(`hero_image`/`gallery_image_1/2/3`) بدل ما تفضل فاضية — من Unsplash (ترخيص مجاني تجاري)،
مستضافة محلياً في `public/images/template-library/{category-slug}/` (80 صورة، ~22
ميجابايت، صفر hotlink خارجي). كل صورة اتحمّلت وتحقّق منها (magic bytes + HTTP 200) قبل
الربط. أي مشروع جديد بياخدها تلقائي، وقابلة للتغيير يدوي زي أي خانة تانية. **ملحوظة:**
كل الـ15 قالب في نفس الفئة بيشاركوا نفس الـ4 صور (زي ما بيشاركوا نفس نص hero_title
افتراضياً) — لو فؤاد حب صور مختلفة لكل قالب داخل الفئة نفسها ده تحسين مستقبلي محتاج
مجهود تصفح/تنزيل أكبر بكتير (300 صورة بدل 80).

**⚠️ تجربة اتعملت واتلغت: معاينة حقيقية 100% بـiframe لكل قالب في /templates (2026-09-21)**
— جرّبنا نستبدل كارت الصورة/اللون التقريبي بـiframe حقيقي بيرندر نفس التصميم والألوان
الفعليين لكل قالب (route+controller method اسمهم كانوا `templates.preview`/
`TemplateController::preview()`، بيستخدم Project/GeneratedSite مؤقتين بالذاكرة مع
SiteRenderer العادي). **رجعناها بعد التجربة** — حتى مع lazy-loading (IntersectionObserver
+ ResizeObserver بدل CSS container queries اللي فشلت الأول)، فتح صفحة فيها 300 قالب يعني
عشرات الـiframes بتتحمّل مرة واحدة (أي حاجة قريبة من الشاشة)، وده تقيل واضح على سيرفر 4
أنوية بس مع pool برق الصغير (`max_children=2`). فؤاد لاحظ تقل حقيقي في السيرفر لحظة فتح
الصفحة. **القرار:** كارت القالب رجع للصورة الحقيقية (نفس صور الفئة) + تدرّج لوني بألوان
القالب + badge باسم التصميم — صفر نداء سيرفر إضافي لكل كارت، بدل محاولة "معاينة حقيقية"
