...
**🖼️ صور حقيقية في مكتبة القوالب (Phase 13، بدأت 2026-09-21)** — فؤاد طلب صور ليها علاقة
بالفئة تتحط كـ default_value لخانات الصور (`hero_image`/`gallery_image_1/2/3`) بدل ما
تفضل فاضية. فئة "مطاعم وكافيهات" خلصت كتجربة أولى (4 صور Unsplash — ترخيص مجاني تجاري —
مستضافة محلياً في `public/images/template-library/{category-slug}/`، مش hotlink خارجي).
باقي الـ19 فئة لسه — كل فئة محتاجة نفس الخطوة (تصفح Unsplash، اختيار صور، تنزيل محلي،
ربطها في `categories()` جوّه `SeedTemplateLibrary.php`).
