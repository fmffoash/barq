...
**⚠️ قاعدة صيانة خامسة (اتحددت 2026-09-21):** أي ملف static عنده اسم ثابت (مش hash-based
زي أصول Vite في `public/build/`) — أشهر مثال `public/js/live-editor.js` و
`public/css/live-editor.css` — **Cloudflare بيكاشه**، فتعديل الملف على السيرفر مش كفاية:
الزوّار (وفؤاد) هيفضلوا شايفين نسخة قديمة لحد ما الكاش ينتهي من نفسه أو حد يعمله purge يدوي
من Cloudflare dashboard. **الحل المطبّق (`site/document.blade.php`):** أي `<link>`/`<script>`
لملف static بالاسم الثابت ده لازم `?v={{ filemtime(public_path('...')) }}` في آخر الرابط —
بيتغيّر تلقائي كل مرة الملف يتعدّل فعلياً، فCloudflare بيعتبره URL جديد كل مرة (cache miss)
بدل ما يفضل يقدّم القديم. **أي ملف static جديد بنفس الطبيعة (اسم ثابت، مش من Vite) لازم
...
**🐛 باج جوهري اتصلح: تعديلات الخط/التصميم بتتحفظ بس مش بتبان (Phase 16، 2026-09-22)** —
فؤاد اشتكى إن "أي تعديل بعمله مبيتعدلش" وإن شكل الخط ثابت مهما غيّره. بعد تحقق شامل (HTML/
CSS/خطوط/Cloudflare كلهم صح 100%) طلع السبب الحقيقي في `public/js/live-editor.js`: دالة
`save()` كانت بتعمل `toast` بس من غير `window.location.reload()` — والخط العام/تخينه/ميله/
حجمه وتبديل القالب وخط/لون الخانة الواحدة **كلهم بيترندروا وقت تحميل الصفحة من السيرفر بس**،
فالتعديل كان بيتخزن صح 100% في الداتا بيز (اتأكد بـtinker) بس شكله على الشاشة مايتغيرش
خالص من غير ريلود. النص لوحده كان بيبان فوراً (contenteditable مباشر) فده كان مخبّي إن باقي
التعديلات "مش شغالة". **الحل:** `save()` بقت بترجع true/false حسب نجاح الحفظ فعلاً (مش زي
الأول، الكارتش كان بيسكت الخطأ ويخلي أي `.then()` بعديها يشتغل حتى لو الحفظ فشل)، ولما تنجح
بيحصل `reload()` تلقائي — في `commitActive()` بس لو الخط/اللون اتغيّر فعلاً (مش لأي تعديل
نص عادي)، وفي فورم "تصميم الموقع" دايماً. اتوتست بتغيير `font_override` عن طريق tinker
...
زوم + سحب + حفظ + ريلود + تأكيد بصري بسكرين شوت كلهم شغالين صح. **المرحلة 3 (ترتيب حر لأي
عنصر، Phase 18، 2026-09-23)** — فؤاد وضّح بعد كده إن قصده الأصلي كان أوسع من الصور: "أي عنصر
في الصفحة (نص وأقسام كمان)"، واختار صراحة موضع حر كامل (نقل+تغيير حجم زي Canva/Wix) بدل تحريك
محدود آمن، مع تأكيد إن صفر أهمية لدعم الموبايل (برق مخصص لمتصفح كمبيوتر بس). التنفيذ: زرار
"📐 ترتيب حر" جديد في المحرر البصري بيفعّل وضع سحب/تكبير مستقل تماماً عن تعديل النص/الصور
العادي، لأي خانة غير `list` (عناصر القايمة بتشارك `data-slot` واحد فمفيش معنى موضع حر لعنصر
متكرر). المكان بيتخزن كنسبة مئوية (`posX`/`posY`/`width`) في نفس `style_overrides_json`
الموجود، `document.blade.php` بيولّد `position:absolute` + `left`/`top`/`width` بنفس آلية
الـCSS المركزية الموجودة من قبل. كل الـ16 layout بقى عندها `position:relative` على الـ
`<section>`، وكل خانة `link` بقى عندها `data-slot` (مكنش موجود خالص قبل كده). **4 باجات
حقيقية** اتكشفت بـPlaywright: (1) خانة أصلها `w-full` بتمتد لعرض القسم كامل بعد
`position:absolute` — الحل: تثبيت العرض الحقيقي وقت `mousedown` مع كل حفظ. (2) الـcontaining
block الحقيقي مش دايماً `<section>` — تصميمات زي gallery فيها حاوية داخلية `position:relative`
أضيق (لعمل z-index فوق صورة الهيرو) — الحل: `sectionRectFor()` في `live-editor.js` بقى
يستخدم `el.offsetParent` بدل `closest('section')` دايماً. (3) الحاوية دي نفسها بتتغيّر
حجمها لحظة ما العنصر بيسيب الـflow (لو حجمها معتمد على محتواها) — قياس واحد وقت `mousedown`
بيبقى قديم لباقي السحب؛ الحل: إعادة القياس في كل `mousemove`/`mouseup` (خفّض الانحراف عن مكان
الماوس من ~39px لـ~7px، وخلّى الموضع بعد الريلود يطابق بالظبط 0px فرق مكان الـ`mouseup`). (4)
عناصر `position:absolute` بترندر فوق أي عنصر `static` تاني بغض النظر عن ترتيب DOM — لو عنصرين
اتحركوا لمكان متقارب، التاني بيغطي الأول ويمنع الدوس عليه (سلوك متوقع لأي أداة موضع حر، مش
باج) — تخفيف جزئي بس (`z-index` مؤقت وقت hover في `live-editor.css`) للتراكب الجزئي، بدون
نظام layers كامل (قرار متعمّد، موثّق في الخطة). اتوتست على تصميمين (classic + gallery، التاني
عشان الـcontaining block المتداخل) + تستات PHPUnit جديدة (`SiteLayoutTest`) لحفظ/رفض القيم
ولإثبات إن الترتيب الحر بيشتغل على أي slot_type (خانة `link` بالتحديد، مش text/image بس).

**📌 دمج PR #1 (Phase 18) وPR #2 (إصلاح CI) — تقرير (2026-09-23):** وقت ما الجلسة اللي عملت
Phase 18 جهّزت PR #1، لاقت `main` اتقدّم من جلسة Claude تانية شغّالة بالتوازي على نفس المشروع
— دمجت نسخة مستقلة بتاعها من المرحلة 1 (تنسيق النص) مباشرة على `main` من غير PR (كوميت
`70279c1`)، ومعاها خطة موثّقة بديلة للمرحلة 3 (نظام CSS Grid بمواضع "snapped" بدل موضع حر
كامل، غير منفّذة خالص، مجرد اقتراح في `docs/rich-text-and-image-editing-plan.md`). ده سبب
merge conflict حقيقي على فرع PR #1. بالمراجعة (مقارنة كل ملف مختلف بـ`git diff`) طلع إن
نسخة `main` من المرحلة 1 subset كامل من الكود الموجود في فرع PR #1 بالفعل (نفس الحل بالحرف
تقريباً، `RichTextSanitizer.php` طلع متطابق 100%) — فالـmerge اتحل لصالح فرع PR #1 في كل
مكان، وخطة الـGrid البديلة اتشالت من التوثيق لأنها بتناقض تأكيد فؤاد الصريح والمباشر (رد
مباشر لي في المحادثة: "عايز رقم 2 ... مش فارق معايا يشتغل علي الموبايل" = موضع حر كامل بالسحب،
مش شبكة). بعد الـmerge، CI طلع أحمر — طلع السبب مش في كود المرحلة 3 خالص: `.github/workflows/
tests.yml` كان بيعمل `composer install` بس وأبداً مش `npm install`/`npm run build`، و
`public/build/` gitignored، فـ`public/build/manifest.json` (لازم لأي `@vite(...)` في أي view)
مكانش موجود خالص وقت الـCI — باج قديم موجود من قبل، اتأكد بإن الـscheduled run اليومي على
`main` نفسه فشل بنفس السبب قبل أي تعديل هنا. اتصلح في PR منفصل (#2،
`.github/workflows/tests.yml`) بدل ما يتحط جوّه PR #1 (عشان مايوسّعش نطاقه من غير داعي) —
اتوتست محلياً بإعادة نفس تسلسل الـCI بالظبط (`npm ci && npm run build` + `php artisan test`)
قبل الدفع. الاتنين (PR #1 وPR #2) اتدمجوا في `main` بإذن صريح من فؤاد.
