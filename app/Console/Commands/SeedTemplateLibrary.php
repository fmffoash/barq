...
// قالب. كل قالب في الفئة بياخد تصميم بصري مختلف تماماً (من الـ 15 تصميم في
// Template::LAYOUTS) ولوحة ألوان مختلفة تماماً (من 15 لوحة) — فمفيش قالبين في نفس الفئة
// شكلهم أو ألوانهم واحدة. المحتوى الافتراضي (نص الهيرو، الخدمات، آراء العملاء...) واحد لكل
// فئة (زي ما الفئة نفسها بتمثّله)، والاختلاف بين الـ 15 قالب جوّه الفئة في الاسم/الشكل/اللون
// — الأدمن بيعدّل النص الفعلي وقت ما يعمل مشروع حقيقي منه. كل النصوص عربية أصلية اتكتبت
...
class SeedTemplateLibrary extends Command
{
    ...
    private const LAYOUT_KEYWORDS = [
        ...
        'glass' => ['فخامة', 'فاخر', 'فاخرة', 'رقي', 'راقي', 'أنيق', 'أناقة', 'حصري', 'حصرية', 'رفاهية', 'استثنائي', 'جودة استثنائية', 'تجربة راقية', 'أنوثة راقية', 'تركيبات نادرة', 'متعة فاخرة', 'رقي في كل قضمة', 'بريق خاص', 'بريق دايم', 'عناية استثنائية', 'رعاية استثنائية', 'انطلاقة عالمية', 'رعاية متكاملة'],
        ...
        'framed' => ['إطار', 'توثيق', 'دقة', 'رسمي', 'احترافي', 'احترافية عالية', 'رفيع', 'تفاصيل دقيقة', 'تشخيص دقيق', 'إصلاح احترافي', 'تنظيم احترافي', 'انطباع احترافي', 'انطباع أول قوي', 'عناية دقيقة', 'تحرير احترافي', 'توقيعك الفريد', 'حماية دائمة', 'التزام', 'امتثال'],
    ...
    public function handle(): int
    {
        ...
        foreach ($this->categories() as $categoryIndex => $category) {
            ...
            foreach ($category['names'] as $templateIndex => $name) {
                // اللون لسه بيتوزّع دورانياً (مفيش شكوى عليه)، بس التصميم بقى بيتاخد حسب
                // قرب معنى اسم القالب من شخصية كل تصميم — راجع matchLayoutsToNames().
                $layout = $layoutForIndex[$templateIndex];
                $palette = $this->palettes[$paletteKeys[($templateIndex + $categoryIndex * 2) % $paletteCount]];
