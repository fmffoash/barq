...
@forelse ($sections as $section)
    ...
    @if ($section['kind'] === 'hero')
        <section id="{{ $section['key'] }}" class="relative grid min-h-[75vh] items-stretch {{ $imageItems->isNotEmpty() ? 'lg:grid-cols-2' : '' }}">
            <div class="flex flex-col items-start justify-center gap-5 px-6 py-16 text-right sm:px-14 {{ $imageItems->isNotEmpty() ? 'order-2 lg:order-1' : 'mx-auto max-w-2xl items-center text-center' }}">
                ...
                @foreach ($linkItems as $item)
                    <a data-slot="{{ $item['slot']->key }}" href="{{ $item['value'] }}" target="_blank" rel="noopener" class="mt-2 inline-block px-8 py-3.5 text-base font-bold transition hover:opacity-90" style="background-color: var(--site-primary); color: var(--site-background);">{{ $item['slot']->label() }}</a>
                @endforeach
            </div>
    ...
    @elseif ($section['kind'] === 'gallery')
        <section id="{{ $section['key'] }}" class="relative grid border-t {{ $imageItems->isNotEmpty() ? 'lg:grid-cols-2' : '' }}" style="border-color: color-mix(in srgb, var(--site-text) 10%, transparent);">
    ...
    @elseif ($section['kind'] === 'list')
        <section id="{{ $section['key'] }}" class="relative border-t px-6 py-14 sm:px-14" style="border-color: color-mix(in srgb, var(--site-text) 10%, transparent);">
    ...
    @elseif ($section['kind'] === 'cta')
        <section id="{{ $section['key'] }}" class="relative border-t px-6 py-16 text-center sm:px-14" style="background-color: var(--site-primary); border-color: color-mix(in srgb, var(--site-text) 10%, transparent);">
            <div class="mx-auto flex max-w-2xl flex-col items-center gap-4">
                ...
                @endforeach
                @foreach ($linkItems as $item)
                    <a data-slot="{{ $item['slot']->key }}" href="{{ $item['value'] }}" target="_blank" rel="noopener" class="mt-2 inline-block px-8 py-3.5 text-base font-bold transition hover:opacity-90" style="background-color: var(--site-background); color: var(--site-primary);">{{ $item['slot']->label() }}</a>
                @endforeach
            </div>
        </section>
    @else
        <section id="{{ $section['key'] }}" class="relative border-t px-6 py-14 text-center sm:px-14" style="border-color: color-mix(in srgb, var(--site-text) 10%, transparent);">
            <div class="mx-auto flex max-w-2xl flex-col gap-4">
                @foreach ($section['items'] as $item)
                    ...
                    @elseif ($item['slot']->slot_type === 'link')<a data-slot="{{ $item['slot']->key }}" href="{{ $item['value'] }}" target="_blank" rel="noopener" class="mx-auto inline-block px-8 py-3 text-base font-semibold transition hover:opacity-90" style="background-color: var(--site-primary); color: var(--site-background);">{{ $item['slot']->label() }}</a>
