        ...
        >
            @if ($section['kind'] === 'hero')
                @foreach ($textItems as $item)
                    @if ($item['slot']->slot_type === 'text')<h1 class="text-4xl font-extrabold sm:text-6xl">{{ $item['value'] }}</h1>@else<p class="max-w-xl text-lg" style="color: var(--site-muted);">{{ $item['value'] }}</p>@endif
                @endforeach
                ...
                @foreach ($listItems as $item)
                    <div class="grid gap-4 sm:grid-cols-2">
                        @foreach ((array) $item['value'] as $listItem)
                            <div class="rounded-2xl border p-5 text-base shadow-sm" style="background-color: color-mix(in srgb, var(--site-background) 30%, transparent); border-color: color-mix(in srgb, var(--site-text) 12%, transparent);">{{ $listItem }}</div>
                        @endforeach
                    </div>
                @endforeach
            @elseif ($section['kind'] === 'cta')
                @foreach ($textItems as $item)
                    @if ($item['slot']->slot_type === 'text')<h2 class="text-3xl font-bold">{{ $item['value'] }}</h2>@else<p style="color: var(--site-muted);">{{ $item['value'] }}</p>@endif
                @endforeach
                @foreach ($linkItems as $item)
                    <a href="{{ $item['value'] }}" target="_blank" rel="noopener" class="mt-2 inline-block rounded-full px-8 py-3.5 text-base font-bold shadow-md transition hover:opacity-90" style="background-color: var(--site-primary); color: var(--site-background);">{{ $item['slot']->label() }}</a>
                @endforeach
