    ...
    <section id="{{ $section['key'] }}" class="px-4 py-14 sm:px-8">
        <div class="mx-auto max-w-5xl {{ $isHeroOrCta ? 'flex flex-col items-center gap-5 text-center' : 'flex flex-col gap-6' }}">
            @if ($section['kind'] === 'hero')
                @foreach ($textItems as $item)
                    @if ($item['slot']->slot_type === 'text')
                        <h1 data-slot="{{ $item['slot']->key }}" class="text-4xl font-extrabold sm:text-6xl" style="color: var(--site-text); text-shadow: 0 0 18px color-mix(in srgb, var(--site-primary) 65%, transparent), 0 0 40px color-mix(in srgb, var(--site-primary) 35%, transparent);">{{ $item['value'] }}</h1>
                    @else
            ...
            @elseif ($section['kind'] === 'cta')
                @foreach ($textItems as $item)
                    @if ($item['slot']->slot_type === 'text')<h2 data-slot="{{ $item['slot']->key }}" class="text-3xl font-bold" style="text-shadow: 0 0 16px color-mix(in srgb, var(--site-primary) 55%, transparent);">{{ $item['value'] }}</h2>@else<p data-slot="{{ $item['slot']->key }}" style="color: var(--site-muted);">{{ $item['value'] }}</p>@endif
                @endforeach
                @foreach ($linkItems as $item)
