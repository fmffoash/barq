            ...
            @if ($section['kind'] === 'hero')
                <div class="flex flex-col items-center gap-5 rounded-[2rem] p-10 text-center shadow-md sm:p-14" style="background-color: var(--site-surface);">
                    @foreach ($textItems as $item)
                        @if ($item['slot']->slot_type === 'text')<h1 data-slot="{{ $item['slot']->key }}" class="text-4xl font-extrabold sm:text-5xl">{{ $item['value'] }}</h1>@else<p data-slot="{{ $item['slot']->key }}" class="max-w-xl text-lg" style="color: var(--site-muted);">{{ $item['value'] }}</p>@endif
                    @endforeach
